...
describe('sent kudos bekijken', () => {
    it('sends a kudo and it should be in outbox', () => {
        ...
        cy.get('[data-cy=Speaker]').get('input[list=Speaker]').type('KobeDehandschutterTest')
        cy.get('[data-cy=Session]').get('input[list=Session]').type('DeBesteSessieTerWereldNatuurlijk')
        cy.get('[data-cy=FAB]').click()
